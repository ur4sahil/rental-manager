-- Wizard loan picker: edit a SPECIFIC loan by id.
--
-- A property can carry more than one loan (1st mortgage + HELOC etc.), added
-- from the Loans page. The wizard was single-loan: it found the property's
-- loan with LIMIT 1 and blind-updated whichever came first, hiding the rest.
-- Now the wizard sends loan.id when editing an existing loan, and this commit
-- targets THAT loan. With no id: 1 existing loan -> update it (unchanged
-- behavior); 0 -> insert; >1 -> insert a new one (can't guess). Body is
-- identical to 20260922140000 except the two DECLAREs and the loan lookup.
...
BEGIN
  ...
  IF v_mode = 'edit' THEN
    ...
    WHERE company_id = v_company_id AND property = v_address AND archived_at IS NULL AND is_final_bill IS NOT TRUE;
  END IF;
  ...
  ELSIF v_mode = 'edit' THEN
    -- Loan step disabled. Only auto-archive when the property has a single
    -- loan; with multiple loans the wizard cannot tell which one to remove,
    -- so it leaves them for the Loans page to manage.
    SELECT count(*) INTO v_loan_count FROM property_loans
     WHERE company_id = v_company_id AND property = v_address AND archived_at IS NULL;
    IF v_loan_count <= 1 THEN
