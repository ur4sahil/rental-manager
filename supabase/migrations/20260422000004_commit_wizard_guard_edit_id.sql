-- commit_property_wizard: guard the property_id_for_edit cast.
--
-- v_property_id_in was unconditionally cast to bigint at the top of
-- the function. If the payload ever carried a UUID there (stale
-- class_id / tenant_id from a previously contaminated wizard_data
-- row, or any other mishap), the cast threw
--   "invalid input syntax for type bigint: '<uuid>'"
-- before the mode check could even run — with mode='fresh' the
-- field should be ignored entirely, not parsed.
--
-- New behavior: only cast when mode='edit' AND the value is a pure
-- digit string. Anything else → treat as fresh, even if the client
-- asked for edit mode. Belt-and-suspenders with the client-side
-- guard that now coerces non-numeric savedPropertyId to null.
