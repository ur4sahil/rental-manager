-- commit_property_wizard: populate recurring_journal_entries.tenant_id
-- on both INSERT and UPDATE for the rent recurring row, plus add a
-- CHECK constraint enforcing (tenant_name IS NULL) = (tenant_id IS
-- NULL). Any future writer that misses one of the two will fail at
-- INSERT time instead of silently NULLing tenant_id and breaking
-- downstream tenants.balance updates.
--
-- Body is identical to 20260424000002 with three additions:
--   • UPDATE at the rent-recurring upsert sets tenant_id = v_tenant_id
--   • INSERT at the rent-recurring upsert lists tenant_id and v_tenant_id
--   • CHECK constraint added at the bottom (NOT VALID first, then
--     VALIDATED — existing rows have already been backfilled by
--     tests/sync-tenant-balances.js Phase 1).
...
-- Invariant: a recurring row is either tenant-scoped (rent — both
-- tenant_name AND tenant_id set) or property-scoped (mortgage,
-- HOA, utilities — both null). Mixed states broke
-- autoPostRecurringEntries' tenant balance update for years.
-- Backfill across all 13 affected companies completed before this
-- ran (tests/backfill-recurring-tenant-id-all-companies.js), so
-- the constraint validates cleanly without NOT VALID.
ALTER TABLE recurring_journal_entries
  DROP CONSTRAINT IF EXISTS recurring_je_tenant_pair_check;
ALTER TABLE recurring_journal_entries
  ADD CONSTRAINT recurring_je_tenant_pair_check
  CHECK ((tenant_name IS NULL OR tenant_name = '') = (tenant_id IS NULL));
