-- commit_property_wizard: two small, targeted changes vs 000010.
--
-- (1) day_of_month clamp is now 1..31 instead of 1..28.
--     autoPostRecurringEntries (src/utils/accounting.js) already
--     computes the actual post day per month via
--     Math.min(stored_day, daysInMonth) — so storing 31 means "last
--     day of the month" and Feb posts on the 28/29th, April on the
--     30th, July on the 31st. The prior 1..28 clamp silently rounded
--     user input down to "28" forever, which is what Sahil flagged
--     on Q9. Clamp still caps at 31 so we never insert a negative or
--     out-of-range day.
--
-- (2) The "existing Monthly rent%%" upsert SELECT now also matches
--     by tenant_name. On a multi-unit property with two separate
--     leases, the prior (property + description LIKE 'Monthly
--     rent%%') LIMIT 1 would return whichever row was created first
--     — the UPDATE then hijacked that row to belong to the wizard's
--     current tenant, silently wiping the other tenant's recurring.
--     With the tenant_name match, each lease has an independent
--     canonical row. Co-tenant leases that share a single joined
--     tenant_name ("Alice / Bob") still resolve to one row.
--
-- Copied wholesale from 20260422000010 with only those two lines
-- changed. Diff-minimal so a future audit can confirm no other body
-- changes vs 000010.
