-- Two-line final bill: the wizard's utilities archive-all (edit mode) must
-- EXCLUDE is_final_bill lines, so a re-save never wipes/duplicates the
-- auto-created final-bill line. This is the commit_property_wizard change the
-- two-line migration (20260922110000) only DOCUMENTED but did not contain.
-- Body = 20260922080000 (current prod) with 'AND is_final_bill IS NOT TRUE'
-- added to the utilities archive-all only (NOT the hoa_payments one).
