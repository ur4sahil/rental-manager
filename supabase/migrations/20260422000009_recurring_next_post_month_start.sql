-- commit_property_wizard: anchor recurring at first-of-next-month,
-- not lease_start + 1 month.
--
-- Product rule clarified:
--   - First entry fires on lease_start (handled by the wizard's manual
--     PRORENT-* / RENT1-* post — prorated if mid-month).
--   - Recurring entries start on day_of_month of the NEXT calendar
--     month after lease_start's month, not lease_start + 1 month.
--     For a Jan 15 lease with day_of_month=1 → Feb 1, not Feb 15.
--   - Explicit recurring.start_date in the wizard still overrides.
--
-- Fix is a single expression change in the v_next_post_date fallback.
-- No other body changes vs migration 20260422000008.
...
BEGIN
  ...
  IF v_is_occupied AND v_tenant_id IS NOT NULL
    ...
    v_next_post_date := COALESCE(
      ...
      -- Default: day_of_month of the month immediately after
      -- lease_start's calendar month. Wizard already posts first
      -- month via PRORENT-* / RENT1-*, so recurring should pick up
      -- from the NEXT month on the day the PM chose — not from
      -- lease_start + N months (which would land mid-month for a
      -- mid-month lease).
      CASE
        WHEN NULLIF(v_tenant->>'lease_start','') IS NOT NULL THEN
