-- commit_property_wizard: fix variable types for UUID-keyed tables.
--
-- The bigint/UUID error we kept seeing wasn't a contaminated payload
-- — it was the RPC itself. Several local variables were declared
-- bigint (leases.id, property_loans.id, property_insurance.id,
-- property_taxes.id, recurring_journal_entries.id), but those
-- columns are UUID in the live schema. The first SELECT ... INTO
-- on a match blew up with:
--   "invalid input syntax for type bigint: '<uuid>'"
-- and aborted the whole commit — even on a "fresh" wizard, if any
-- prior partial-commit row existed for the same company+address.
--
-- Fix: redeclare v_lease_id, v_existing_lease_id, v_existing_loan_id,
-- v_existing_ins_id, v_existing_tax_id, v_existing_recur_id,
-- v_existing_mort_id as uuid. v_property_id + v_tenant_id stay
-- bigint (those columns really are bigint). Body is otherwise
-- identical to migration 20260422000004.
...
BEGIN
  ...
  -- Only consider property_id_for_edit when the caller actually
  -- asked for edit mode AND the value is pure digits. Anything else
  -- means "no live property" — force fresh-commit mode.
