-- commit_property_wizard: anchor recurring cadence at lease_start.
--
-- Product decision: the PM's lease-start date is already the source
-- of truth for when rent should begin. Making recurring.start_date
-- a "must-remember-or-no-catch-up" field was friction for no reason.
--
-- New default when the PM doesn't set recurring.start_date:
--   next_post_date = lease_start + freq months
--
-- Why +freq and not lease_start itself: the wizard already manually
-- posts a first-month rent JE (prorated when lease_start is
-- mid-month) with its own RENT1-* / PRORENT-* reference. Setting
-- next_post_date = lease_start would make autoPostRecurringEntries
-- also post the first month as a RECUR-* JE — the two references
-- don't collide, so the idempotency check doesn't catch it.
-- Anchoring one period forward skips exactly the month the wizard
-- already covered.
--
-- Effect on a Jan 1 2026 lease onboarded Apr 22 2026:
--   wizard posts: Jan 1 RENT1 (full month, lease starts day 1)
--   default:      next_post_date = Feb 1 2026
--   autoPost runs (called from wizard + App.js login):
--     Feb 1 <= today → posts RECUR Feb, advances next_post_date → Mar 1
--     Mar 1 <= today → posts RECUR Mar, next_post_date → Apr 1
--     Apr 1 <= today → posts RECUR Apr, next_post_date → May 1
--     May 1 > today  → exit
--   final: 4 JEs (Jan RENT1 + Feb/Mar/Apr RECUR), next_post_date May 1.
--
-- PMs who type an explicit recurring.start_date still win — the
-- COALESCE below picks their value first.
--
-- Also folds in the fix from 20260422000006 (unchanged body
-- elsewhere): v_class_id text, _wizard_get_tenant_ar drops the
-- acct_accounts.tenant_id write.
...
BEGIN
  ...
  IF v_is_occupied AND v_tenant_id IS NOT NULL
    ...
    -- Anchor: PM's explicit start_date → else lease_start + one
    -- frequency period (the month after the wizard's manual first-
    -- month post) → else today + 1m as a last-resort guard.
