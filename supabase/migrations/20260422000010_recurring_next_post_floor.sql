-- commit_property_wizard: floor user's recurring.start_date at the
-- default (first-of-next-month after lease_start's calendar month).
--
-- Bug observed: PM typed recurring.start_date = 2026-01-01 to match a
-- lease_start of 2026-01-01. The wizard posted RENT1-2026-01-01 for
-- that first month, and then autoPostRecurringEntries — seeing
-- next_post_date = 2026-01-01 — also posted RECUR-2026-01. Same
-- month billed twice (different reference prefixes, no cross-dedup).
--
-- Fix: when the PM's explicit start_date falls at or before the
-- default (first-of-next-month), clamp to the default. PMs can still
-- push the date further into the future (delayed-start or
-- migration-mode setups) but can never anchor earlier than the
-- wizard's manual first-month post already covers.
--
-- Tradeoff: a PM who wants the recurring stream to INCLUDE the
-- lease-start month now must not post a first-month rent separately
-- (which the wizard does automatically). Given the wizard always
-- posts first-month via PRORENT/RENT1, that's the correct guarantee.
--
-- No other body changes vs 20260422000009.
