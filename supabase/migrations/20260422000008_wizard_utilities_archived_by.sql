-- commit_property_wizard: stop writing archived_by on utilities.
--
-- The utilities table has archived_at but no archived_by column (the
-- other wizard-scoped tables — hoa_payments, property_loans,
-- property_insurance, property_taxes — all have both, which is what
-- tricked me). Edit-mode re-runs of the wizard did:
--   UPDATE utilities SET archived_at = now(), archived_by = v_caller_email
-- and Postgres rejected the whole RPC with
--   column "archived_by" of relation "utilities" does not exist
-- so Save Changes on any existing property blew up with PM-8002.
--
-- Fix: only write archived_at on utilities. Keep archived_by on the
-- other tables (those columns do exist). No other body changes vs
-- migration 20260422000007.
