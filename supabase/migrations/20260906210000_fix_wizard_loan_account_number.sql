-- commit_property_wizard wrote three columns that do not exist on
-- property_loans: account_number_encrypted, account_number_iv and
-- account_number_salt. So filling in a loan made the ENTIRE property
-- wizard fail -- ten steps of data, the property, tenant, lease,
-- utilities, HOA, insurance and tax all lost -- with the message
-- "A required database table is missing. Please contact support."
--
-- Found by driving the wizard end to end for the first time: the commit
-- returned HTTP 400 with
--   column "account_number_encrypted" of relation "property_loans" does not exist
--
-- It also never wrote account_number, which the table DOES have and
-- which the client sends as plaintext (Properties.js:670), so the
-- account number the user typed was discarded regardless.
--
-- The RPC now writes account_number. Nothing else in the function is
-- changed; this is the definition as it stood with those three
-- references replaced by the real column.
