...
cd tests && npm run test:routes                       # every role page/nav id has a component; app-written columns exist
...
- **A column the client writes must actually exist** — PostgREST returns 400 for an unknown column, and the app's `try/catch` + `data || []` idiom turns that into an empty list rather than an error. Three columns were missing in production: `utilities.archived_by` and `inspections.archived_at/by` meant deleting a property silently left its utilities and inspections live (while the dialog promised otherwise), and `tenants.updated_at` meant the concurrent-edit guard never fired once. `cd tests && npm run test:routes` guards these plus the rule that every role page and nav id resolves to a real component (`autopay` did not, and silently rendered the Dashboard)
- **Tenant-facing policies must key on `tenant_id`, never `tenant` name** — 5 same-name tenant groups exist in production, so a name-keyed policy shows one tenant another's rows. `get_tenant_id(company_id)` is the id-based counterpart to `get_tenant_name()`. `documents` and `payments` now carry a backfilled `tenant_id`; rows whose name was ambiguous were deliberately left null and are therefore invisible to tenants (fail closed) until a human assigns them
