...
export async function autoPostJournalEntry({ date, description, reference, property, lines, status = "posted", companyId }) {
  ...
  //   - unique_je_number_per_company (company_id, number)
  //   - idx_je_company_reference_unique (company_id, reference)
  // Number is generated by the next_je_number RPC, which takes MAX
  // over rows matching ^JE-\d+$ (so hash-format legacy JEs can't
  // poison the sequence). MAX doesn't lock, so concurrent inserts
  // can still race — the retry loop re-calls the RPC each attempt
  // so the second-place writer sees the first's committed row.
  // A reference collision means the dedup guard did its job: caller
  // asked to post the same deterministic reference twice, which is
  // a no-op, not an error worth retrying.
  let jeRow = null, jeErr = null;
  for (let attempt = 0; attempt < 5; attempt++) {
  const { data: jeNumber, error: numErr } = await supabase.rpc("next_je_number", { p_company_id: cid });
  if (numErr || !jeNumber) { pmError("PM-4002", { raw: numErr || { message: "next_je_number returned null" }, context: "next_je_number RPC" }); return null; }
