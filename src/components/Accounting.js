...
// Imported for this file's OWN use. A re-export alone does not bring a
// name into scope -- without this every acctFmt/getPLData call in here
// becomes undefined, which is the single most expensive bug class in this
// repo (a swallowed ReferenceError that fails silently forever).
import {
  ACCOUNT_SUBTYPES, ACCOUNT_TYPES, DEBIT_NORMAL, DEFAULT_ACCOUNT_SUBTYPES,
  DEFAULT_ACCOUNT_TYPES, PERIODS, acctFmt, acctFmtDate, acctToday,
...
// Moved to utils/acctReports.js -- arithmetic, not a component.
// Re-exported here because other modules import ACCOUNT_TYPES and
// validateJE from this file; repointing their imports is a separate
// tidy-up, not part of this move.
export {
  ACCOUNT_SUBTYPES,
  ACCOUNT_TYPES,
  DEBIT_NORMAL,
...
export function AccountLedgerView({ accountIds, accounts, journalEntries, title, onClose, onViewJE, linesLoaded = true, linesFailed = false }) {
  ...
  //
  // This used to build one flat list with a single running balance across
  // every selected account. For a total spanning 46 accounts that is
  // worse than useless: the rows interleave by date, and the balance
  // column adds cash to receivables to fixed assets, so no number in it
  // means anything. A combined ledger has to read as a stack of ledgers
  // -- the shape of the QuickBooks exports Sahil supplied.
  //
  // The direction is taken per account too. The old code read
  // getNormalBalance(acctMap[ids[0]].type) once and applied it to all of
  // them, so in a mixed selection every account after the first ran
  // backwards.
  ...
  const allLines = groups.flatMap(g => g.rows);
  const grandDr = groups.reduce((t, g) => t + g.totalDr, 0);
  const grandCr = groups.reduce((t, g) => t + g.totalCr, 0);
...
export function AcctChartOfAccounts({ accounts, journalEntries, onAdd, onUpdate, onToggle, onDelete, onOpenLedger, showToast }) {
  ...
  <DataTable
    columns={[
      { key: "number", label: "Number", className: a => (`tnum text-xs text-neutral-400 ${a._isSubAccount ? "pl-8 pr-5" : ""}`),
        render: a => (<>{a._isSubAccount ? "└ " : ""}{a.code || "—"}</>) },
      { key: "name", label: "Name", className: a => (`${a._isSubAccount ? "text-sm text-neutral-600" : "font-medium"} ${!a.is_active ? "text-neutral-400 line-through" : a._isSubAccount ? "" : "text-neutral-800"}`),
        render: a => (<>
          <LedgerLink ids={[a.id]} title={(a.code ? a.code + " " : "") + a.name} onOpenLedger={onOpenLedger} className="">{a.name}</LedgerLink>
        </>) },
      { key: "subtype", label: "Subtype", className: "text-xs text-neutral-400",
        render: a => (<>{a.subtype || ""}</>) },
      { key: "balance", label: "Balance", align: "right", className: a => (`tnum text-sm ${a.computedBalance < 0 ? "text-danger-600" : "text-neutral-800"}`),
        render: a => (<>{acctFmt(a.computedBalance, true)}</>) },
      { key: "actions", label: "Actions", align: "center", className: "flex items-center gap-2 justify-center",
        render: a => (<>
...
function AcctJEFormModal({ mode, je, seed, accounts, classes, tenants = [], vendors = [], companyId, showToast, onClose, onSave }) {
  ...
  {showNewAcct !== null && (
  <div className="bg-brand-50 rounded-xl p-3 mb-3 border-2 border-brand-400 shadow-pop">
...
export function AcctJournalEntries({ accounts, journalEntries, classes, tenants = [], vendors = [], onAdd, onUpdate, onPost, onVoid, onReverse, companyId, onOpenLedger, initialViewJEId, autoOpenAdd, showToast, onCloseJEDetail }) {
  ...
  <DataTable
    columns={[
      ...
      { key: "source", label: "Source", className: "text-xs text-neutral-400",
        render: je => (<>{refLabel(je.reference)}</>) },
      { key: "status", label: "Status",
        render: je => (<><AcctStatusBadge status={je.status} /></>) },
      { key: "amount", label: "Amount", align: "right", className: "tnum text-sm font-semibold",
        render: je => { const total = (je.lines || []).reduce((s,l) => s + safeNum(l.debit), 0); return (<>{acctFmt(total)}</>); } },
  ...
  <DataTable
    columns={[
      { key: "account", label: "Account",
        render: l => (<>
          {(() => { const acct = accounts.find(a => a.id === l.account_id); const code = acct?.code || ""; const name = l.account_name || acct?.name || "Unknown Account"; return <>{code && <span className="tnum text-xs text-neutral-400 mr-1">{code}</span>}{name}</>; })()}
        </>) },
      { key: "class", label: "Class", className: "text-xs",
        render: l => { const cls = classes.find(c => c.id === l.class_id); return (<>
          {cls ? <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full" style={{background:cls.color}} />{cls.name}</span> : "—"}
        </>); } },
      { key: "tenant_vendor", label: "Tenant/Vendor", className: "text-xs text-neutral-500",
        render: l => (<>
          {l.entity_name ? <span>{l.entity_type === "vendor" ? "V: " : "C: "}{l.entity_name}</span> : "—"}
        </>) },
      { key: "memo", label: "Memo", className: "text-xs text-neutral-400",
        render: l => (<>{l.memo || "—"}</>) },
      { key: "debit", label: "Debit", align: "right", className: "tnum",
        render: l => (<>{safeNum(l.debit) > 0 ? acctFmt(l.debit) : ""}</>) },
      { key: "credit", label: "Credit", align: "right", className: "tnum",
        render: l => (<>{safeNum(l.credit) > 0 ? acctFmt(l.credit) : ""}</>) },
    ]}
    rows={(modal.je.lines || [])}
    rowKey={l => l.id}
    empty="Nothing to show"
...
export function AcctReports({ linesLoaded = true, linesFailed = false, accounts, journalEntries, classes, companyName, companyId, userProfile, showToast, onOpenLedger, onRefresh }) {
  ...
  // Accountants reconcile against a full chart, so a zero-balance
  // account still has to be visible -- its absence is what you are
  // checking. Hidden by default because the common case is reading,
  // not reconciling.
...
.grid{display:grid}.grid-cols-4{grid-template-columns:repeat(4,1fr)}`;
  ...
  async function exportExcel() {
    ...
    // ===================== PROFIT & LOSS =====================
    if (id === "pl") {
      const d = getPLData(accounts, journalEntries, start, end, classFilter || null, showZeros);
    ...
    // ===================== P&L COMPARISON =====================
      ...
      const d1 = getPLData(accounts, journalEntries, start, end, classFilter || null, showZeros);
    ...
    // ===================== TRIAL BALANCE =====================
      ...
      tbData.filter(a => showZeros || a.debitBalance !== 0 || a.creditBalance !== 0).forEach(a => {
  ...
  if (activeView === "catalog") {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        ...
        <div key={r.id} onClick={() => openReport(r)} className="group cursor-pointer border border-neutral-200 rounded-xl p-4 hover:border-brand-300 hover:shadow-pop transition-all bg-white">
      ...
      <div>{customReports.length === 0 ? <div className="text-center py-12 text-neutral-400"><span className="material-icons-outlined text-4xl mb-2 block">tune</span><p className="text-sm">No saved report configurations yet.</p><p className="text-xs mt-1">Open any report, configure filters, then click "Save Config" in the toolbar.</p></div> : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {customReports.map(c => (
          <div key={c.id} className="group border border-neutral-200 rounded-xl p-4 hover:border-brand-300 hover:shadow-pop transition-all bg-white">
            <div className="flex items-start justify-between">
  ...
  const plData = getPLData(accounts, journalEntries, start, end, classFilter || null, showZeros);
  ...
  <div>
    ...
    <div className="bg-neutral-50 rounded-xl p-4 mb-4 flex flex-wrap gap-3 items-end">
      ...
      {/* Zero-balance accounts. Reconciling against a chart of accounts
          means checking that an account is empty, which you cannot do if
          empty accounts are hidden. Off by default: the common case is
          reading a report, not reconciling one. */}
      <label className="flex items-center gap-1.5 text-xs text-neutral-500 whitespace-nowrap self-end pb-1.5 ml-auto">
        <Checkbox checked={showZeros} onChange={e => setShowZeros(e.target.checked)} className="accent-brand-600" />
        Show zero balances
      </label>
      ...
      <DataTable
        columns={[
          { key: "account", label: "Account", className: "text-neutral-700",
            render: a => (<>{a.code ? a.code + " " : ""}{a.name}</>) },
          { key: "debit", label: "Debit", align: "right", className: "tnum",
            render: a => (<>{a.debitBalance > 0 ? acctFmt(a.debitBalance) : ""}</>) },
          { key: "credit", label: "Credit", align: "right", className: "tnum",
            render: a => (<>{a.creditBalance > 0 ? acctFmt(a.creditBalance) : ""}</>) },
        ]}
    ...
    {reportId === "ar_aging_summary" && (<div>
      ...
      <DataTable
        columns={[
          { key: "tenant", label: "Tenant", className: "text-neutral-700 whitespace-nowrap",
            render: t => (<>{t.tenant}</>) },
          { key: "current", label: "Current", align: "right", className: "tnum whitespace-nowrap",
            render: t => (<>{t.current ? acctFmt(t.current) : ""}</>) },
          { key: "1_30", label: "1-30", align: "right", className: "tnum whitespace-nowrap",
            render: t => (<>{t.days30 ? acctFmt(t.days30) : ""}</>) },
          { key: "31_60", label: "31-60", align: "right", className: "tnum whitespace-nowrap",
            render: t => (<>{t.days60 ? acctFmt(t.days60) : ""}</>) },
          { key: "61_90", label: "61-90", align: "right", className: "tnum whitespace-nowrap",
            render: t => (<>{t.days90 ? acctFmt(t.days90) : ""}</>) },
          { key: "91", label: "91+", align: "right", className: "tnum whitespace-nowrap",
            render: t => (<>{t.over90 ? acctFmt(t.over90) : ""}</>) },
          { key: "total", label: "Total", align: "right", className: "tnum font-semibold whitespace-nowrap",
            render: t => (<>{acctFmt(t.current + t.days30 + t.days60 + t.days90 + t.over90)}</>) },
        ]}
      ...
      <DataTable
        columns={[
          ...
          { key: "entry", label: "Entry", className: "text-xs text-brand-600 tnum",
            render: t => (<>{t.jeNumber||""}</>) },
          { key: "account", label: "Account", className: "text-neutral-700",
            render: t => (<>{t.accountName}</>) },
          { key: "description", label: "Description", className: "text-xs text-neutral-500",
        ...
        rows={getTransactionsByDate(start, end)}
        rowKey={t => t.id}
        empty="Nothing to show"
      />
    </div>)}
    ...
    {reportId === "account_list" && (<div>
      ...
      <DataTable
        columns={[
          { key: "code", label: "Code", className: "tnum text-xs text-neutral-600",
            render: a => (<>{a.code||"—"}</>) },
          { key: "tenant_vendor", label: "Tenant/Vendor", className: "text-neutral-800",
            render: a => (<>{a.name}</>) },
    ...
    {reportId === "expenses_by_category" && (<div>
      ...
      {(() => { const data = getExpensesByCategory(start, end); return (<DataTable
        columns={[
          { key: "category", label: "Category", className: "text-neutral-700",
            render: a => (<>
              <LedgerLink ids={[a.id]} title={a.name} onOpenLedger={onOpenLedger} className="">{a.name}</LedgerLink>
            </>) },
          { key: "amount", label: "Amount", align: "right", className: "tnum",
            render: a => (<>{acctFmt(a.amount)}</>) },
          { key: "of_total", label: "% of Total",
            render: a => (<>
              <div className="flex items-center gap-2"><div className="flex-1 bg-neutral-100 rounded-full h-2"><div className="bg-brand-500 rounded-full h-2" style={{width: Math.min(100, a.percentage) + "%"}} /></div><span className="text-xs text-neutral-500 w-8">{a.percentage}%</span></div>
            </>) },
        ]}
        rows={data}
        rowKey={a => a.id}
      ...
      {(() => { const data = getNOIByProperty(start, end); return (<DataTable
        columns={[
          ...
          { key: "expenses", label: "Expenses", align: "right", className: "tnum text-danger-600",
            render: r => (<>{acctFmt(r.expenses)}</>) },
          { key: "noi", label: "NOI", align: "right", className: r => (`tnum font-bold ${r.noi < 0 ? "text-danger-600" : "text-success-700"}`),
            render: r => (<>{acctFmt(r.noi)}</>) },
          { key: "margin", label: "Margin", align: "right", className: "text-sm",
            render: r => (<>{r.noiMargin}%</>) },
        ]}
        rows={data}
        rowKey={r => r.id}
        empty="Nothing to show"
      ...
      {(() => { const data = getVacancyReport(); return data.length === 0 ? <EmptyState size="compact" title={"No vacant properties"} /> : (<DataTable
        columns={[
          ...
          { key: "days_vacant", label: "Days Vacant", align: "right", className: "tnum",
            render: r => (<>{r.daysVacant}</>) },
          { key: "est_lost_revenue", label: "Est. Lost Revenue", align: "right", className: "tnum text-danger-600",
            render: r => (<>{acctFmt(r.estimatedLost)}</>) },
        ]}
        rows={data}
        rowKey={r => r.id}
        empty="Nothing to show"
    ...
    {reportId === "work_orders_summary" && (() => { const data = getWorkOrderSummary(start, end); return (<div>
      ...
      <DataTable
        columns={[
          ...
          { key: "status", label: "Status",
            render: w => (<>
              <span className={`text-xs px-2 py-0.5 rounded-full ${w.status==="open"?"bg-warn-100 text-warn-700":w.status==="in_progress"?"bg-highlight-100 text-highlight-700":"bg-success-100 text-success-700"}`}>{w.status}</span>
            </>) },
          { key: "cost", label: "Cost", align: "right", className: "tnum",
            render: w => (<>{w.cost ? acctFmt(w.cost) : "—"}</>) },
          { key: "days", label: "Days", align: "right", className: "tnum text-neutral-500",
            render: w => (<>{w.daysOpen}</>) },
        ]}
        rows={data.items}
        rowKey={w => w.id}
      ...
      {(() => { const data = getOpenInvoices(asOfDate); return data.length === 0 ? <EmptyState size="compact" title={"No unpaid charges"} /> : (<DataTable
        columns={[
          ...
          { key: "original", label: "Original", align: "right", className: "tnum",
            render: r => (<>{acctFmt(r.originalAmount)}</>) },
          { key: "paid", label: "Paid", align: "right", className: "tnum text-success-600",
            render: r => (<>{acctFmt(r.amountPaid)}</>) },
          { key: "due", label: "Due", align: "right", className: "tnum font-semibold text-danger-600",
            render: r => (<>{acctFmt(r.amountDue)}</>) },
          { key: "days", label: "Days", align: "right", className: r => (`tnum ${r.daysOutstanding > 60 ? "text-danger-600 font-bold" : r.daysOutstanding > 30 ? "text-warn-600" : ""}`),
            render: r => (<>{r.daysOutstanding}</>) },
        ]}
        rows={data}
        rowKey={r => r.id}
    ...
    {reportId === "collections" && (<div>
      ...
      {(() => { const data = getCollectionsReport(asOfDate); return data.length === 0 ? <EmptyState size="compact" title={"No outstanding balances"} /> : (<DataTable
        columns={[
          ...
          { key: "total_owed", label: "Total Owed", align: "right", className: "tnum font-bold text-danger-600",
            render: r => (<>{acctFmt(r.total)}</>) },
          { key: "severity", label: "Severity", align: "center",
            render: r => (<>
              <span className={`text-xs px-2 py-0.5 rounded-full ${r.severity==="critical"?"bg-danger-100 text-danger-700":r.severity==="warning"?"bg-warn-100 text-warn-700":"bg-neutral-100 text-neutral-500"}`}>{r.severity}</span>
            </>) },
        ]}
        rows={data}
        rowKey={r => r.id}
    ...
    {reportId === "customer_balance_detail" && (<div>
      ...
      <DataTable
        columns={[
          ...
          { key: "debit", label: "Debit", align: "right", className: "tnum",
            render: tx => (<>{tx.debit > 0 ? acctFmt(tx.debit) : ""}</>) },
          { key: "credit", label: "Credit", align: "right", className: "tnum",
            render: tx => (<>{tx.credit > 0 ? acctFmt(tx.credit) : ""}</>) },
          { key: "balance", label: "Balance", align: "right", className: tx => (`tnum font-semibold ${tx.balance < 0 ? "text-positive-600" : ""}`),
            render: tx => (<>{acctFmt(tx.balance, true)}</>) },
        ]}
        rows={t.transactions}
        rowKey={tx => tx.id}
        empty="Nothing to show"
    ...
    {reportId === "expenses_by_vendor" && (<div>
      ...
      {(() => { const data = getExpensesByVendor(start, end); return (<DataTable
        columns={[
          ...
          { key: "of_total", label: "% of Total",
            render: r => (<>
              <div className="flex items-center gap-2"><div className="flex-1 bg-neutral-100 rounded-full h-2"><div className="bg-notice-500 rounded-full h-2" style={{width: Math.min(100, r.percentage) + "%"}} /></div><span className="text-xs text-neutral-500 w-8">{r.percentage}%</span></div>
            </>) },
        ]}
        rows={data}
        rowKey={r => r.id}
      ...
      {(() => { const data = getSecurityDepositLedger(asOfDate); return data.length === 0 ? <EmptyState size="compact" title={"No security deposits held"} /> : (<><DataTable
        columns={[
          ...
          { key: "returned", label: "Returned", align: "right", className: "tnum text-danger-600",
            render: r => (<>{r.returned > 0 ? acctFmt(r.returned) : ""}</>) },
          { key: "net_held", label: "Net Held", align: "right", className: "tnum font-bold",
            render: r => (<>{acctFmt(r.netHeld)}</>) },
        ]}
        rows={data}
        rowKey={r => r.id}
    ...
    {reportId === "late_fees" && (<div>
      ...
      {(() => { const data = getLateFeeReport(start, end); return data.length === 0 ? <EmptyState size="compact" title={"No late fees in this period"} /> : (<DataTable
        columns={[
          { key: "tenant", label: "Tenant", className: "text-neutral-700",
            render: r => (<>{r.tenant}</>) },
          { key: "assessed", label: "Assessed", align: "right", className: "tnum",
            render: r => (<>{acctFmt(r.feesAssessed)}</>) },
          { key: "collected", label: "Collected", align: "right", className: "tnum text-success-600",
            render: r => (<>{acctFmt(r.feesCollected)}</>) },
          { key: "outstanding", label: "Outstanding", align: "right", className: "tnum font-bold text-danger-600",
            render: r => (<>{r.feesOutstanding > 0 ? acctFmt(r.feesOutstanding) : ""}</>) },
      ...
      {(() => { const data = getOwnerDistributions(start, end); return data.length === 0 ? <EmptyState size="compact" title={"No distributions in this period"} /> : (<DataTable
        columns={[
          ...
          { key: "description", label: "Description", className: "text-neutral-700",
            render: r => (<>{r.description}</>) },
          { key: "amount", label: "Amount", align: "right", className: "tnum font-semibold",
            render: r => (<>{acctFmt(r.amount)}</>) },
        ]}
        rows={data}
        rowKey={r => r.id}
    ...
    {reportId === "rent_collection" && (<div>
      ...
      <DataTable
        columns={[
          ...
          { key: "collected", label: "Collected", align: "right", className: "tnum text-success-600",
            render: r => (<>{acctFmt(r.collected)}</>) },
          { key: "outstanding", label: "Outstanding", align: "right", className: "tnum text-danger-600",
            render: r => (<>{r.outstanding > 0 ? acctFmt(r.outstanding) : ""}</>) },
          { key: "rate", label: "Rate", align: "right",
            render: r => (<>{r.collectionRate}%</>) },
        ]}
      ...
      {(() => { const data = getAPAgingData(asOfDate); const vendors = Object.entries(data.byVendor).filter(([,d]) => Math.abs(d.total) > 0.01); return vendors.length === 0 ? <EmptyState size="compact" title={"No outstanding payables"} /> : (<DataTable
        columns={[
          ...
          { key: "current", label: "Current", align: "right", className: "tnum",
            render: ([vendor, d]) => (<>{d.current ? acctFmt(d.current) : ""}</>) },
          { key: "1_30", label: "1-30", align: "right", className: "tnum",
            render: ([vendor, d]) => (<>{d.days30 ? acctFmt(d.days30) : ""}</>) },
          { key: "31_60", label: "31-60", align: "right", className: "tnum",
            render: ([vendor, d]) => (<>{d.days60 ? acctFmt(d.days60) : ""}</>) },
          { key: "61_90", label: "61-90", align: "right", className: "tnum",
            render: ([vendor, d]) => (<>{d.days90 ? acctFmt(d.days90) : ""}</>) },
          { key: "91", label: "91+", align: "right", className: "tnum",
            render: ([vendor, d]) => (<>{d.over90 ? acctFmt(d.over90) : ""}</>) },
          { key: "total", label: "Total", align: "right", className: "tnum font-semibold",
            render: ([vendor, d]) => (<>{acctFmt(d.total)}</>) },
        ]}
        rows={vendors}
        rowKey={(row, i) => i}
        footer={[{ label: "TOTALS", strong: true, cells: [
      ...
      {(() => { const data = getUnpaidBills(); return data.length === 0 ? <EmptyState size="compact" title={"No unpaid bills found"} /> : (<DataTable
        columns={[
          ...
          { key: "description", label: "Description", className: "text-xs text-neutral-500 truncate max-w-48",
            render: r => (<>{r.description}</>) },
          { key: "ref", label: "Ref", className: "tnum text-xs text-brand-600",
            render: r => (<>{r.jeNumber||""}</>) },
          { key: "amount", label: "Amount", align: "right", className: "tnum font-semibold",
            render: r => (<>{acctFmt(r.amount)}</>) },
        ]}
      ...
      {auditData.length === 0 ? <EmptyState size="compact" title={"No audit entries in this period"} /> : (<DataTable
        columns={[
          ...
          { key: "module", label: "Module", className: "text-xs",
            render: r => (<>
              <span className="bg-neutral-100 text-neutral-600 px-1.5 py-0.5 rounded">{r.module}</span>
            </>) },
          { key: "action", label: "Action", className: "text-xs",
            render: r => (<>
              <span className={`px-1.5 py-0.5 rounded ${r.action==="create"?"bg-success-100 text-success-700":r.action==="delete"?"bg-danger-100 text-danger-600":r.action==="update"?"bg-info-100 text-info-700":"bg-neutral-100 text-neutral-600"}`}>{r.action}</span>
            </>) },
          { key: "details", label: "Details", className: "text-xs text-neutral-500 max-w-64 truncate",
            render: r => (<>{r.details}</>) },
        ]}
        rows={auditData}
        rowKey={r => r.id}
      ...
      {reconData.length === 0 ? <EmptyState size="compact" title={"No reconciliations found"} /> : (<DataTable
        columns={[
          { key: "period", label: "Period", className: "text-neutral-700",
            render: r => (<>{r.period}</>) },
          { key: "bank_balance", label: "Bank Balance", align: "right", className: "tnum",
            render: r => (<>{acctFmt(safeNum(r.bank_ending_balance))}</>) },
          { key: "book_balance", label: "Book Balance", align: "right", className: "tnum",
            render: r => (<>{acctFmt(safeNum(r.book_balance))}</>) },
          { key: "difference", label: "Difference", align: "right", className: r => (`tnum ${Math.abs(safeNum(r.difference)) < 0.01 ? "text-success-600" : "text-danger-600"}`),
            render: r => (<>{acctFmt(safeNum(r.difference))}</>) },
          { key: "status", label: "Status", align: "center",
            render: r => (<>
              <span className={`text-xs px-2 py-0.5 rounded-full ${r.status==="reconciled"?"bg-success-100 text-success-700":"bg-warn-100 text-warn-700"}`}>{r.status}</span>
            </>) },
        ]}
        rows={reconData}
        rowKey={r => r.id}
    ...
    {reportId === "budget_vs_actual" && (<div>
      ...
      {(() => { const data = getBudgetVsActual(start, end); const hasAnyBudget = data.some(a => a.budget > 0); return !hasAnyBudget ? <EmptyState size="compact" title={'No budgets set. Click "Edit Budgets" to set monthly amounts.'} /> : (<DataTable
        columns={[
          { key: "account", label: "Account", className: "text-neutral-700",
            render: a => (<>{a.name}</>) },
          { key: "actual", label: "Actual", align: "right", className: "tnum",
            render: a => (<>{acctFmt(a.amount)}</>) },
          { key: "budget", label: "Budget", align: "right", className: "tnum text-neutral-400",
            render: a => (<>{acctFmt(a.budget)}</>) },
          { key: "variance", label: "Variance ($)", align: "right", className: a => { const favorable = a.isExpense ? a.variance < 0 : a.variance > 0; return (`tnum font-semibold ${favorable ? "text-success-600" : "text-danger-600"}`); },
            render: a => (<>{acctFmt(a.variance, true)}</>) },
          { key: "variance", label: "Variance (%)", align: "right", className: a => { const favorable = a.isExpense ? a.variance < 0 : a.variance > 0; return (`${favorable ? "text-success-600" : "text-danger-600"}`); },
            render: a => (<>{a.variancePct > 0 ? "+" : ""}{a.variancePct}%</>) },
        ]}
        rows={data.filter(a => a.budget > 0)}
        rowKey={a => a.id}
        empty="Nothing to show"
      />); })()}
    </div>)}
...
export function Accounting({ companySettings = {}, companyId, activeCompany, addNotification, userProfile, userRole, showToast, showConfirm, initialAction, initialTab }) {
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <div className="bg-white rounded-xl border border-neutral-200 p-4 hover:shadow-pop transition-shadow">
  ...
  <div className="flex flex-row flex-nowrap gap-3 mb-6 overflow-x-auto">
    <button onClick={() => setActiveTab("journal")} className="shrink-0 inline-flex items-center gap-2 bg-white border border-neutral-200 rounded-xl px-4 py-3 text-sm text-neutral-700 hover:border-brand-300 hover:shadow-card transition-all whitespace-nowrap">
    ...
    <button onClick={() => setActiveTab("recurring")} className="shrink-0 inline-flex items-center gap-2 bg-white border border-neutral-200 rounded-xl px-4 py-3 text-sm text-neutral-700 hover:border-brand-300 hover:shadow-card transition-all whitespace-nowrap">
    ...
    <button onClick={() => setActiveTab("reports")} className="shrink-0 inline-flex items-center gap-2 bg-white border border-neutral-200 rounded-xl px-4 py-3 text-sm text-neutral-700 hover:border-brand-300 hover:shadow-card transition-all whitespace-nowrap">
