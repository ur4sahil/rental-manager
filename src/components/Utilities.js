...
function Utilities({ addNotification, userProfile, userRole, companyId, showToast, showConfirm }) {
  ...
  <div><label className="text-xs font-medium text-neutral-400 mb-1 block">Username</label><Input name="util_portal_user" autoComplete="off" value={form.username||""} onChange={e => setForm({...form, username: e.target.value})} /></div>
  <div><label className="text-xs font-medium text-neutral-400 mb-1 block">Password</label><Input type="password" name="util_portal_pass" autoComplete="new-password" value={form.password||""} onChange={e => setForm({...form, password: e.target.value})} /></div>
  </div></div>
  </div>
  <div className="flex gap-2 mt-3">
